...
async def client(act_command: list[str], wasm_path: Path, tmp_path: Path):
    ...
    This is the fixture that differs from crypto's reference: `filesystem`
    needs a `wasi:filesystem` grant to do anything, and it is stateful across
    *files*, not just across calls within a process. The nine old hurl files
    shared a single `mktemp -d` for the entire suite run, so a file one of
    them wrote was visible to whichever ran after it — real cross-test
    coupling, not just an artifact of the old harness.
    ...
    the identical directory. That gives every test its own process AND its
    own directory: strictly tighter isolation than the hurl suite had, not a
...
def expect_error():
    ...
    read-a-missing-file test does not. Both are handled here so callers need
    not care.
    """
